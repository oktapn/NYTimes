...
public class RVListArticle extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    public final int VIEW_ITEM = 1;
    public final int VIEW_PROG = 0;

    private List<Doc> contentItems;

    public void setContentItems(List<Doc> contentItems) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof RVListArticle.TextViewHolder) {
            final Context context = holder.itemView.getContext();
            final String imgurl = contentItems.get(position).getMultimedia().size() != 0 ? contentItems.get(position).getMultimedia().get(0).getUrl() : "-";
            final String snippet = contentItems.get(position).getSnippet();
            final String tgl = contentItems.get(position).getPubDate() == null ? " " : getDateMonthParsefromString(contentItems.get(position).getPubDate());
            final String headline = contentItems.get(position).getHeadline().getMain();
    ...
    public void addItem(final Doc newmenus, final int position) {
